...
class PhoenixHbasePool(PhoenixHbase):
    ...
    def __get_coloumn(self, table_name):
        '''
        phoenix 查询的结果是一个列表, 若需要字典需要自行整合
        :param table_name:
        :return:
        '''
        self.cursor.execute(f"SELECT ORDINAL_POSITION, COLUMN_NAME FROM SYSTEM.\"CATALOG\" WHERE TABLE_NAME = \'{table_name}\'")
        position_list = self.cursor.fetchall()

        for each in position_list:
            self.position_dict[each[0]] = each[1]

    def search_all_from_phoenix(self, table_name=None, output_field=None, limit_num=None, offset_num=None,
        ...
        if not output_field:
            if not self.position_dict and dict_status:
                self.__get_coloumn(table_name=table_name)
        ...
        if iter_status:
            return self.__search_iter(curs=self.cursor, output_field=output_field, dict_status=dict_status)
        else:
            result = list()
            one_result = self.__search_iter(curs=self.cursor, output_field=output_field, dict_status=dict_status)
    ...
    def delete_from_phoenix(self, where_condition=None):
        """
        删除数据
    ...
    def upsert_to_phoenix(self, data_list):
        # pho_logger = Logger().logger
        self.count = 0
        for data in data_list:
            ...
            self.cursor.setinputsizes(52428800)
            self.cursor.executemany(sql, batch_value_)
            self.conn.commit()
            self.count += 1
    ...
    # 插入单条
    def upsert_to_phoenix_by_one(self, data, table_name=None):
        """
        插入单条数据
        :param data: 需要插入的数据
        ...
        if table_name:
            ...
            sql = 'upsert into \"{}\" {} values(?{})'.format(self.table_name, batch_key_,
                                                             (",?" * (len(batch_key) - 1)))
        # print(sql)
        batch_value_.append(batch_value)
        # print(batch_value)
        self.cursor.setinputsizes(52428800)
        self.cursor.setoutputsize(52428800)
        self.cursor.executemany(sql, batch_value_)
        self.conn.commit()
        self.count += 1
        return self.count
...
if __name__ == '__main__':
    ...
    # 删除表
    # test1_.drop_table_phoenix(connection=connection, table_name="CHA_BRANCH_WECHAT")
