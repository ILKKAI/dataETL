...
from tools.req_for_api import req_for_serial_number, req_for_file_save, req_for_file_download
from tools.web_api_of_baidu import get_lat_lng, haversine
...
class PhoenixHbase(object):
    ...
    def __init__(self, table_name):
        ...
        jvm_path = jpype.getDefaultJVMPath()  # 检索到默认的或第一次找到的JVM库的路径
        jpype.startJVM(jvm_path, args)  #  Starts a Java Virtual Machine
        self.count = 0
    ...
    def __connect_to_phoenix(self):
        connection = jaydebeapi.connect('org.apache.phoenix.jdbc.PhoenixDriver',
                                        'jdbc:phoenix:storage01,storage02,crawler01:2181:/hbase',  # 指定zookeeper 的 IP:port
                                        # 'jdbc:phoenix:zookeeper01,zookeeper02,zookeeper03:2181:/hbase',
                                        ['', ''],
                                        self.phoenix_client_jar, # 指定jar 包路径
                                        )
        return connection
    ...
    def search_all_from_phoenix(self, connection, table_name=None, output_field=None, limit_num=None, offset_num=None,
                                iter_status=True, dict_status=False, where_condition=None, group_condition=None):
        """
        查询
...
if __name__ == '__main__':
    ...
    result_generator = test1_.search_all_from_phoenix(connection=connection, dict_status=True, output_field='REGIST_CODE_',
        where_condition="""""", group_condition=""" group by REGIST_CODE_, PRO_NAME_ having count(*) > 1 """)
    while True:
        try:
            ...
            try:
                regist_code = data.get('REGIST_CODE_')
                print(data.get('REGIST_CODE_'))
                curs.execute(f'''select ID_ from CRMLCCP where REGIST_CODE_ = '{data.get('REGIST_CODE_')}'   limit 1''')
                id = curs.fetchone()[0]
                # test1_.delete_from_phoenix(connection=connection, where_condition=f""" ID_ = '{id}' """)
