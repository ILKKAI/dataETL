...
COMMON_DICT = lambda: dict(
    DEAL_STATUS_="DRAFT",
    PUBLISH_STATUS_="N",
    BANK_CODE_="ICBC",  # 银行编码
    BANK_NAME_="中国工商银行",  # 银行名称
...
def aggre_for_name():
    agg_data = collection.aggregate([
        {"$match": {"ENTITY_CODE_": "XYK_CP_ZGGSYH"}},
...
def shuffle_data(data, count):
    ...
    img_response = requests.get(url=img_url, headers=header, verify=False)
