...
def aggre_for_name():
    agg_data = collection.aggregate([
        {"$match": {"ENTITY_CODE_": "XYK_CP_ZGGDYH"}},
        {"$project": {"_id": 0, "CARD_NAME_": 1}},
        {"$group": {"_id": "$CARD_NAME_"}}
    ])

    global name_list
    name_list = [each["_id"][:4] for each in agg_data]
...
def shuffle_data(data, count):
    ...
    surface_dict["LEVEL_"] = data["LEVEL_"]  # 卡等级
...
def main():
    ...
    for name in name_list:
        ...
        data_iter = data_from_mongo(find_query={"$and": [
            {"ENTITY_CODE_": "XYK_CP_ZGGDYH"},
            {"CARD_NAME_": {"$regex": f"^{name}"}}
        ]},
                                    collection=collection)
        # print(data_iter.count())
