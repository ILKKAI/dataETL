...
# 攀枝花市商业银行网站 510400PZHCCB

# 392 条  13条无 TITLE_ 字段  多为重复数据和图片
# 父级界面有时间
...
def data_shuffle(data):
    fina_result = ""
    if "TITLE_" in data:
        result = re.findall(r"中标供应商[:：](\w*[(（《]?\w*[)）》]?\w*)", data["CONTENT_"])
        if len(result) == 1:
...
def run():
    script = GenericScript(entity_code="510400PZHCCB", entity_type="CommonBidding")
