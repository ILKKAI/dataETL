...
def data_shuffle(data):
    fina_result = ""
    if "一标段" in data["TITLE_"] or "三标段" in data["TITLE_"]:
        fina_result = ""
    elif "结果" in data["TITLE_"] or "中标" in data["TITLE_"]:
        result = re.findall(r"供应商.*?排名[|]?(\w*[(（]?\w*[)）]?\w*)", data["CONTENT_"])
        if result:
            if len(result) == 1:
...
def run():
    script = GenericScript(entity_code="140000JSYH", entity_type="CommonBidding")
