...
# 枣庄银行网站 277100ZZYH
# 439 为"" 2 条 无 NOTICE_TIME 字段 待清洗 有父级
# 无 WIN_CANDIDATE_ 字段  1788
...
def data_shuffle(data):
    ...
    if "结果" in data["TITLE_"] or "中标" in data["TITLE_"]:
        # 过滤包组情况
        if "B包" in data["CONTENT_"] or "标段二" in data["CONTENT_"]:
            fina_result = ""
        else:
            result = re.findall(r"[一二三四五六七八九十]、中标情况(.*?)[一二三四五六七八九十]|[一二三四五六七八九十]、成交单位(.*?)[一二三四五六七八九十]", data["CONTENT_"])
            ...
            fina_result = fina_result.replace("：", "")
            fina_result = fina_result[:-1]
...
def run():
    script = GenericScript(entity_code="277100ZZYH", entity_type="CommonBidding")
