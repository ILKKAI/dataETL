...
# 中国政府采购招标网网站 86ZGZFZBCG
# NOTICE_TIME_ 5 条 为 "" 可在父级获取
# 招标公告 没有 中标人 有 WIN_CANDIDATE_ 字段 1458
import re
import time
        ...
        if data["NOTICE_TIME_"]:
            if isinstance(data["NOTICE_TIME_"], str):
                if ("-" in data["NOTICE_TIME_"]) and ("{" not in data["NOTICE_TIME_"]) and (
                    ...
                    time_result = re.findall(r"20[1-3][0-9]-[1-3][0-9]", data["NOTICE_TIME_"])
                    if time_result:
                        data["NOTICE_TIME_"] = time_result[0]

            else:
                time_result = re.findall(r"20[1-3][0-9]-[1-3][0-9]", str(data["NOTICE_TIME_"]))
                if time_result:
                    data["NOTICE_TIME_"] = time_result[0]
                else:
                    data["NOTICE_TIME_"] = ""
...
def run():
    script = GenericScript(entity_code="86ZGZFZBCG", entity_type="CommonBidding")
