...
def data_shuffle(data):
    pattern1 = re.compile(r'中标人：(.*?)\|投标报价')
    pattern2 = re.compile(r'中标人名称及其价格\|(.*?)，中标价格')
    pattern3 = re.compile(r'第一中标候选人：(.*?)，中标金额')

    WIN_candidate = ""
    if "结果" in data["TITLE_"] or "中标" in data["TITLE_"]:
        if "候选" not in data["TITLE_"]:
            # print(data["CONTENT_"])
            ...
            handle_list = pattern3.findall(data["CONTENT_"], re.S)
            if handle_list:
                WIN_candidate = handle_list[0]
...
def run():
    script = GenericScript(entity_code="350000FJNX", entity_type="CommonBidding")
