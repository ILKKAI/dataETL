...
# 2 条 ['370600YTYH.CONTENT.NOTICE_TIME_', '2016-02-29']  可父级
...
def data_shuffle(data):
    pattern = re.compile(r'成交情况：\|包号.*\|(.*?公司).*采购人', re.S)

    WIN_candidate = ""
    if "成交" in data["TITLE_"]:
        if "成交入围" not in data["CONTENT_"]:
            # print(data["CONTENT_"])

            handle_list = pattern.findall(data["CONTENT_"])
...
def run():
    script = GenericScript(entity_code="370600YTYH", entity_type="CommonBidding")
