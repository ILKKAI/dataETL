...
# 柳州银行网站 450200LZYH
# 2 条 ['450200LZYH.CONTENT.NOTICE_TIME_', '2018-08-31'] 可父级20181123
# 73 条数据 无 WIN_CANDIDATE_ 字段 已完成
...
def data_shuffle(data):
    ...
    result = re.findall(r"中标供应商名称[:：]\|?(\w*[(（《]?\w*[)）》]?\w*)", data["CONTENT_"])
    if result:
        fina_result = result[0]

    if not result:
        result = re.findall(r"第一中标\|?候选人[:：|]\w*[|](\w*[(（《]?\w*[)）》]?\w*)", data["CONTENT_"])
        if result:
            fina_result = result[0]
...
def run():
    script = GenericScript(entity_code="450200LZYH", entity_type="CommonBidding")
