...
# 重庆国际投资咨询集团有限公司网站 500000CQIIC

# 7 条数据 无 WIN_CANDIDATE_ 字段
# 时间没问题
...
def data_shuffle(data):
    fina_result = ""
    result = re.findall("拟?中标人[:：|](\w*[(（]?\w*[)）]?\w*)", data["CONTENT_"])
    if result:
        fina_result = result[0]

    else:
        result = re.findall(r"第一[中入][标围]候选人[:：|](\w*[(（]?\w*[)）]?\w*)", data["CONTENT_"])
        if result:
            fina_result = result[0]
...
def run():
    script = GenericScript(entity_code="500000CQIIC", entity_type="CommonBidding")
