    ...
    for data in mongo_data_list:
        ...
        if re_data["PERIOD_CODE_"]:
            if "月" in re_data["PERIOD_CODE_"] or "年" in re_data["PERIOD_CODE_"]:
                ...
                re_data["PERIOD_CODE_"] = re_data["PERIOD_CODE_"][:index_year] + "-" + re_data["PERIOD_CODE_"][
                                                                         index_year + 1:index_moutn] + "-" + \
...
def run():
    ...
    script = GenericScript(entity_code="XLCJGSNEWS", entity_type="NEWS_FINASSIST")
    ...
    # pd.DataFrame(data_list).to_csv('E:\\NEWS_CLEAN_\\XLCJGSNEWS.csv', encoding="utf_8_sig")
    # print("文件导出成功")
