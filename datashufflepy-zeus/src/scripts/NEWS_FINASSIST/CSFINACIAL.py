...
def data_shuffle(mongo_data_list):
    ...
    for data in mongo_data_list:
        ...
        # 新闻标题-标记的乱码内容为空
        if data["TITLE_"]:
            if data["TITLE_"].startswith('ÃÏÍí'):
                re_data["TITLE_"] = ""
            else:
                re_data["TITLE_"] = data["TITLE_"]
            if "È" in data["TITLE_"] or "É" in data["TITLE_"]:
                re_data["TITLE_"] = ""
        else:
            re_data["TITLE_"] = ""
        ...
        if data["CONTENT_"]:
            ...
            if index != -1:
                ...
                if data["CONTENT_"].startswith('¡¡¡¡'):
                    ...
                    re_data["CONTENT_"] = clean_Contetn_(re_data["CONTENT_"])
            else:
                if data["CONTENT_"].startswith('¡¡¡¡'):
                    re_data["CONTENT_"] = ""
                elif ".TRS" in data["CONTENT_"]:
        ...
        re_data["ID_"] = data["ENTITY_CODE_"] + "_" + str(9999999999 - int(float(data["DEALTIME_"]))) + "_" + str(hash_title)
...
def run():
    ...
    script = GenericScript(entity_code="CSFINACIAL", entity_type="NEWS_FINASSIST")

    # 调用 GenericScript.data_from_mongo() 方法获取数据
    mongo_data_list = script.data_from_mongo()

    data_list = data_shuffle(mongo_data_list)
    # pd.DataFrame(data_list).to_csv('E:\\NEWS_CLEAN_\\CSFINACIAL.csv', encoding="utf_8_sig")
    # print("1")
