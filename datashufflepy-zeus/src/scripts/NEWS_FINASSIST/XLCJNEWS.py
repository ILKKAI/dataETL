    ...
    for data in mongo_data_list:
        ...
        if data["CONTENT_"]:
            ...
            if index != -1:
                ...
                if "视频加载中" in data["CONTENT_"]:
                    index_1 = data["CONTENT_"].find("(jQuery);|")
                    if index_1:
                        re_data["CONTENT_"] = data["CONTENT_"][index_1 + 10:]
                if ".weiboListBox" in data["CONTENT_"]:
                    index_2 = data["CONTENT_"].find("|}|});|});|")
                    re_data["CONTENT_"] = data["CONTENT_"][index_2+11:]
            else:
                if ".ct_hqi" in data["CONTENT_"]:
                    re_data["CONTENT_"] = clean_Contetn_(data["CONTENT_"])
                    if "视频加载中" in data["CONTENT_"]:
        ...
        if "window.s"in re_data["CONTENT_"]:
            index_3 = re_data["CONTENT_"].find("true|}|")
            re_data["CONTENT_"] = re_data["CONTENT_"][index_3+7:]
...
def run():
    ...
    script = GenericScript(entity_code="XLCJNEWS", entity_type="NEWS_FINASSIST")
    ...
    # pd.DataFrame(data_list).to_csv('E:\\NEWS_CLEAN_\\XLCJNEWS.csv', encoding="utf_8_sig")
    # print("文件导出成功")
