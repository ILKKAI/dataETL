...
def data_shuffle(mongo_data_list):
    ...
    for data in mongo_data_list:
        ...
        # 新闻标题
        if data["TITLE_"]:
            if "È" in data["TITLE_"] or "Ê" in data["TITLE_"] or "«" in data["TITLE_"] or "»" in data["TITLE_"]:
                re_data["TITLE_"] = ""
            else:
        ...
        if data["DATA_SOURCE_"]:
            if "来源：" in data["DATA_SOURCE_"]:
                data["DATA_SOURCE_"] = data["DATA_SOURCE_"][3:]
...
def run():
    ...
    # pd.DataFrame(data_list).to_csv('E:\\NEWS_CLEAN_\\CSNEWS.csv', encoding="utf_8_sig")
    # print("文件导出成功")
