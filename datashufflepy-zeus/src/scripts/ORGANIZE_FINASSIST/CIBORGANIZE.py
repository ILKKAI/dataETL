# -*- coding: utf-8 -*-

# 兴业银行网点 CIBORGANIZE  无经纬度
# 已处理

import hashlib
import re

from scripts import GenericScript


def data_shuffle(data, province_list, city_list, area_list):
    data_list = list()
    re_data = dict()
    prov_n = ""
    prov_c = ""
    city_n = ""
    city_c = ""

    # 省级信息
    for city in city_list:
        if city["NAME_"][:-1] == data["CITY_"]:
            prov_c = city["PARENT_"]
            break
    for prov in province_list:
        if prov["NAME_"][:-1] == data["CITY_"]:
            prov_n = prov["NAME_"]
            prov_c = prov["CODE_"]
            city_n = prov["NAME_"]
            city_c = prov["CODE_"]
            break
        elif prov["CODE_"] == prov_c:
            prov_n = prov["NAME_"]
            break

    # first_shuffle = re.findall(r"\|(\w*[支分]行\w*\|.*?\|?\w*[省市区县道路街][^|]{3,}?)\|", data["CONTENT_"])
    first_shuffle = re.findall(r"\|(\w*[支分]行\w*\|.*?)\|\w*[支分]行\w*\|", data["CONTENT_"])
    if not first_shuffle:
        first_shuffle = re.findall(r"\|(\w*[支分]行\w*\|.*)\|", data["CONTENT_"])

    for first in first_shuffle:
        area_n = ""
        area_c = ""
        name_ = ""
        addr_ = ""

        tel_list = re.findall(r"([^|]*[0-9]{0,4}-?[0-9]{7,8}[^|]*)", first)
        tel = ",".join(tel_list)
        business_time_list = re.findall(r"[^|0-9]*[周双][一休末六日][^|]*|[^|0-9]*[正暂][常停]营业[^|]*|"
                                        r"[0-9][:：][0-9]{2}-{1,2}[0-9]{1,2}[:：][0-9]{2}", first)
        business_time = ",".join(business_time_list)

        second_shuffle = first.split("|")
        name_ = second_shuffle[0]
        addr_ = second_shuffle[1]

        if "狮河" in addr_:
            addr_ = addr_.replace("狮河", "浉河")

        # 市级信息
        for city in city_list:
            if city["PARENT_"] == prov_c:
                if city["NAME_"] in addr_:
                    city_n = city["NAME_"]
                    city_c = city["CODE_"]
        if not city_c:
            city_n = data["CITY_"]
            for city in city_list:
                if city["NAME_"][:2] == city_n:
                    city_n = city["NAME_"]
                    city_c = city["CODE_"]
            continue

        # 区县级信息
        for area in area_list:
            if area["PARENT_"] == city_c:
                if area["NAME_"] in addr_:
                    area_n = area["NAME_"]
                    area_c = area["CODE_"]
        if not area_c:
            for area in area_list:
                if area["CODE_"][:2] == prov_c[:2]:
                    if area["NAME_"] in addr_:
                        area_n = area["NAME_"]
                        area_c = area["CODE_"]
        if area_c:
            if area_c[:4] != city_c[:4]:
                if "北京" in city_n or "天津" in city_n or "上海" in city_n or "重庆" in city_n:
                    pass
                else:
                    city_c = area_c[:4] + "00"
                    for city in city_list:
                        if city["CODE_"] == city_c:
                            city_n = city["NAME_"]

        if name_ == "分行营业部":
            name_ = city_n + name_

        # 地址清洗
        if prov_n in addr_:
            pass
        elif prov_n[:-1] in addr_[:len(prov_n)]:
            addr_ = addr_[:len(prov_n)].replace(prov_n[:-1], prov_n) + addr_[len(prov_n):]
        elif prov_n[:4] in addr_[:len(prov_n)]:
            addr_ = addr_[:len(prov_n)].replace(prov_n[:4], prov_n) + addr_[len(prov_n):]
        elif prov_n[:3] in addr_[:len(prov_n)]:
            addr_ = addr_[:len(prov_n)].replace(prov_n[:3], prov_n) + addr_[len(prov_n):]
        elif prov_n[:2] in addr_[:len(prov_n)]:
            addr_ = addr_[:len(prov_n)].replace(prov_n[:2], prov_n) + addr_[len(prov_n):]
        else:
            addr_ = prov_n + addr_

        if city_n in addr_[:len(prov_n) + len(city_n)]:
            addr_ = addr_
        elif city_n[:-1] in addr_[:len(prov_n) + len(city_n)]:
            addr_ = addr_[:len(prov_n) + len(city_n)].replace(
                city_n[:-1], city_n) + addr_[len(prov_n) + len(city_n):]
        elif city_n[:4] in addr_[:len(prov_n) + len(city_n)]:
            addr_ = addr_[:len(prov_n) + len(city_n)].replace(
                city_n[:4], city_n) + addr_[len(prov_n) + len(city_n):]
        elif city_n[:3] in addr_[:len(prov_n) + len(city_n)]:
            addr_ = addr_[:len(prov_n) + len(city_n)].replace(
                city_n[:3], city_n) + addr_[len(prov_n) + len(city_n):]
        elif city_n[:2] in addr_[:len(prov_n) + len(city_n)]:
            addr_ = addr_[:len(prov_n) + len(city_n)].replace(
                city_n[:2], city_n) + addr_[len(prov_n) + len(city_n):]
        else:
            addr_ = addr_[:len(prov_n)] + city_n + addr_[len(prov_n):]

        # # 添加分行编码
        # branch_code = None
        # for i in range(1, 10000):
        #     branch_code = "CIB" + "_" + area_c + "_" + "00000"
        #     branch_code = branch_code[:len(branch_code) - len(str(i))] + "{}".format(i)
        #     if branch_code in branch_code_list:
        #         continue
        #     else:
        #         branch_code_list.append(branch_code)
        #         break

        # "C"
        hash_m = hashlib.md5()
        hash_m.update(name_.encode("utf-8"))
        hash_title = hash_m.hexdigest()
        re_data["ID_"] = (data["ENTITY_CODE_"] + "_" +
                          str(9999999999 - int(float(data["DEALTIME_"]))) + "_" + str(hash_title))
        re_data["BANK_CODE_"] = "CIB"
        re_data["BANK_NAME_"] = data["ENTITY_NAME_"][:-2]
        re_data["CREATE_TIME_"] = data["DATETIME_"]
        re_data["AREA_CODE_"] = area_c
        re_data["UNIT_CODE_"] = "CIB" + "_" + city_c

        # "F"
        re_data["ADDR_"] = addr_
        re_data["CITY_CODE_"] = city_c
        re_data["CITY_"] = city_n
        re_data["LAT_"] = ""
        re_data["LNG_"] = ""
        re_data["NAME_"] = name_
        re_data["PROVINCE_CODE_"] = prov_c
        re_data["PROVINCE_NAME_"] = prov_n
        re_data["DISTRICT_CODE_"] = area_c
        re_data["DISTRICT_NAME_"] = area_n
        re_data["ENTITY_CODE_"] = data["ENTITY_CODE_"]
        re_data["DEALTIME_"] = data["DEALTIME_"]
        re_data["URL_"] = data["URL_"]
        re_data["TEL_"] = tel
        re_data["BUSINESS_HOURS_"] = business_time

        # "S"
        re_data["STATUS_1"] = ""

        data_list.append(re_data)

    return data_list


def run():
    script = GenericScript(entity_code="CIBORGANIZE", entity_type="ORGANIZE_FINASSIST")
    mongo_data_list = script.data_from_mongo()
    province_list, city_list, area_list, dir_area_list = script.area_from_mysql()
    batch_list = data_shuffle(mongo_data_list, province_list, city_list, area_list)


if __name__ == '__main__':
    run()
