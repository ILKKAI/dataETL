...
class GenericScript(object):
    ...
    def do_insert(self, fina_shuffle, network_status, remove_id):
        ...
        for each in fina_shuffle:
            ...
            # id_list.append(each['DATA_'][0]['DATA_']['ID_'])
    ...
    def shuffle_data(self, data):
        ...
        # 判断部分实体不执行 data_shuffle, 单独进行例如微博类型的清洗, 通用清洗脚本
            ...
            try:
                if "ORGANIZE" in data["ENTITY_CODE_"] or 'WD_TY' in data["ENTITY_CODE_"]:
                ...
                self.logger.exception(f"2.4--err:实体脚本错误."
                                      ...
                                      f" error: {e}.")
                raise Exception(e)
            else:  # 如果这里出来的是列表也可以处理, 在通用清洗脚本处理为单个data, 在回调通用清洗脚本
                if re_data:
    ...
    def insert_data(self, fina_shuffle, remove_id, table_name):
        # 向 HBase 中插入一条
        try:
            # if isinstance(fina_shuffle, list):
            #     fina_shuffle = fina_shuffle[0]['DATA_']
            p_count = self.p_client.upsert_to_phoenix_by_one(connection=self.connection, data=fina_shuffle, table_name=table_name)
    ...
    def main(self):
        ...
        for i in range(self.limit_number):
            ...
            if not data:
                break
            if self.param_dict.get('entityType') == 'JRCP_JJ':
                if len(re.findall('JRCP_JJ_TTJJ', str(deepcopy(reset_data)))) > 4:
                    continue
            # 初始化网络升量状态
            ...
            if self.verify_exist_in_data(data):
                ...
                # if self.exist_in_hbase(data=data):
                    # 数据清洗
                    try:
            ...
            try:
                ...
                del reset_data["_id"]
                # 这里的逻辑需要修改
                status = self.old_client.all_to_mongodb(collection=self.old_collection, insert_list=[reset_data])
                self.logger.info(f"4--success:插入备份数据库成功."
                                 f" 原始数据 collection = {self.m_client.mongo_collection}."
                                 f" ENTITY_CODE_ = {data.get('ENTITY_CODE_', 'None')};"
                                 f" 原始数据 _id = {data.get('_id')};"
                                 f" HBase 数据 ID_ = {' '.join(id_list)};"
                                 f" 备份数据 URL_ = {data.get('URL_')}.")
            except Exception as e:
