...
def data_shuffle(data, province_list, city_list, area_list):
    ...
    data["ADDR_"] = re.findall(r'地址：([\w\S]+)，', addr)[0] if re.findall(r'地址：([\w\S]+)，', addr) else ''
    # '地址：石家庄市裕华区谈固东街150号，电话：0311-85081812'
    data["TEL_"] = re.findall(r'电话：([\w\d\-]*)', addr)[0] if re.findall(r'电话：([\w\d\-]*)', addr) else ''
...
if __name__ == '__main__':
    ...
    main_mongo = MongoClient(entity_code="WD_TY_HEBYH_GW_ALL", mongo_collection="WD_TY")
