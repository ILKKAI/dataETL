    ...
    if not data["AREA_NAME_"]:
        for prov in province_list:
            if prov["NAME_"] in data["ADDR_"] or prov["NAME_"] in data["NAME_"]:
                ...
                break
            elif prov["NAME_"][:-1] in data["ADDR_"] or prov["NAME_"][:-1] in data["NAME_"]:
                data["PROVINCE_NAME_"] = prov["NAME_"]
                data["PROVINCE_CODE_"] = prov["CODE_"]
                break
            elif prov["NAME_"][:2] in data["ADDR_"] or prov["NAME_"][:2] in data["NAME_"]:
                data["PROVINCE_NAME_"] = prov["NAME_"]
                data["PROVINCE_CODE_"] = prov["CODE_"]
                break
        ...
        for city in city_list:
            if city["NAME_"] in data["ADDR_"] or city["NAME_"] in data["NAME_"]:
                ...
                break
            elif city["NAME_"][:-1] in data["ADDR_"] or city["NAME_"][:-1] in data["NAME_"]:
                data["CITY_NAME_"] = city["NAME_"]
                data["CITY_CODE_"] = city["CODE_"]
                break
            elif city["NAME_"][:2] in data["ADDR_"] or city["NAME_"][:2] in data["NAME_"]:
                data["CITY_NAME_"] = city["NAME_"]
                data["CITY_CODE_"] = city["CODE_"]
                break
...
def data_shuffle_city(data, province_list, city_list, area_list):
    re_data = dict()
    prov_n = ""
    prov_c = ""
    city_n = ""
...
if __name__ == '__main__':
    main_mongo = MongoClient(entity_code="SPDBORGANIZE", mongo_collection="WD_TY")
    sc = GenericScript
    # Mysql connection
